...
#     ret = network_fn(ray_batch, **kwargs)
    for k, val_ss in ret.items():
        tf.debugging.check_numerics(val_ss, 'output {}'.format(k))
    return ret
...
def batchify_rays(rays_flat, chunk=1024*32, network_fn=None, **kwargs):
    ...
    for i in range(0, rays_flat.shape[0], chunk):
        ...
        for k, val_sk in ret.items():
            if k not in all_ret:
                all_ret[k] = val_sk
            else:
                all_ret[k] = tf.concat([all_ret[k], val_sk], axis=0)
    return all_ret
...
def train():
    ...
    if use_batching:
        ...
        rays = [get_rays_np(height, width, focal, px) for px in poses[:, :3, :4]]
    ...
    for i in range(start, n_iters):
        ...
        if use_batching:
            ...
            batch = rays_rgb[i_batch:i_batch+num_rand]  # [B, 2+1, 3*?]
            ...
            if num_rand is not None:
                ...
                select_inds = np.random.choice(
                    coords.shape[0], size=[num_rand], replace=False)
